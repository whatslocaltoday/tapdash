...
class Dashboard extends CI_Controller {
    ...
    public function dashboard() {
        ...
        $data = array("total_view" => $total_view, "total_lead" => $total_lead, "cTr" => $cTr, "total_trfc" => $total_trfc, "result_Graphdata" => $result_Graphdata, "result_acc_data" => $result_acc_data, "globalRank" => $globalRank, "counteryRank" => $counteryRank, "counteryCode" => $counteryCode, "uniq_avg_page" => $uniq_avg_page);
    ...
    public function user_profile() {
        ...
        if ($ch_permsn == '0' || $user_id === NULL) {
            ...
            foreach ($result as $row) {
                $data = array("id" => $row->id, "f_name" => $row->f_name, "l_name" => $row->l_name, "gender" => $row->gender, "email" => $row->u_email, "data_web_sel" => $data_web_sel, "message_display" => "");
            }
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
    ...
    public function user_registration_show() {
        ...
        if ($ch_permsn == '0') {
            ...
            $data = array("admin_type" => $adm_typ, "web_proj" => $data_web, "f_name" => $this->input->post('f_name'), "l_name" => $this->input->post('l_name'), "email" => $this->input->post('email'), "user_type" => $this->input->post('user_type'), "menu_option" => $this->input->post('menu_option'), "admin_status" => $this->input->post('admin_status'), "message_display" => "");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
    ...
    public function new_user_registration() {
        ...
        if ($ch_permsn == '0' || $selcd_pro == '') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array("web_proj" => $data_web, "admin_type" => $adm_typ, 'f_name' => $this->input->post('f_name'), 'l_name' => $this->input->post('l_name'), 'email' => $this->input->post('email'), "gender" => $this->input->post('gender'), "user_type" => $this->input->post('user_type'), "permsn_name" => $this->input->post('permsn_name'), "admin_status" => $this->input->post('admin_status'), "message_display" => "");
                $this->load->view('template/header');
                $this->load->view('template/sidebar');
                $this->load->view('admin_user/admin-registration', $data);
                $this->load->view('template/footer');
                ...
                $data1 = array('f_name' => $this->input->post('f_name'), 'l_name' => $this->input->post('l_name'), 'email' => $this->input->post('email'), "gender" => $this->input->post('gender'), "user_type" => $this->input->post('user_type'), "permsn_name" => $permsn_name, "selcd_proj" => $selcd_pro, "admin_status" => $this->input->post('admin_status'), 'cdate' => date('Y-m-d'));
                $result = $this->Admintype_model->registration_insert($data1);
                if ($result == TRUE) {
                    $data2 = array("web_proj" => $data_web, "admin_type" => $adm_typ, "f_name" => "", "l_name" => "", "email" => "", "user_type" => "", "permsn_name" => "", "admin_status" => "", "message_display" => "Registration Successfully!");
                    $this->load->view('template/header');
                    $this->load->view('template/sidebar');
    ...
    public function list_admin_user() {
        ...
        if ($ch_permsn == '0') {
            ...
            $config = array('base_url' => base_url('admin/Dashboard/list_admin_user'), 'per_page' => 10, 'total_rows' => $all_count, 'full_tag_open' => '<ul class="pagination">', 'full_tag_close' => '</ul>', 'first_link' => false, 'last_link' => false, 'first_tag_open' => '<li class="page-link">', 'first_tag_close' => '</li>', 'prev_link' => 'Previous ', 'prev_tag_open' => '<li class="page-link">', 'prev_tag_close' => '</li>', 'next_link' => ' Next', 'next_tag_open' => '<li>', 'next_tag_close' => '</li>', 'last_tag_open' => '<li>', 'last_tag_close' => '</li>', 'cur_tag_open' => '<li class="page-item active"><a class="page-link">', 'cur_tag_close' => '</a></li>', 'num_tag_open' => '<li class="page-link">', 'num_tag_close' => '</li>',);
    ...
    public function view_edit_admin_user() {
        ...
        if ($ch_permsn == '0' || $user_id === NULL) {
            ...
            foreach ($result as $row) {
                $data = array("admin_type" => $adm_typ, "web_proj" => $data_web, "web_proj_sel" => $data_web_sel, "user_type" => $row->user_role_id, "id" => $row->id, "f_name" => $row->f_name, "l_name" => $row->l_name, "gender" => $row->gender, "email" => $row->u_email, "per_list" => $row->aclmaster, "per_name" => $per_name, "menu_option" => $row->acluser, "admin_status" => $row->flag, "message_display" => "");
            }
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
    ...
    public function edit_admin_user() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array("id" => $this->input->post('id'), "f_name" => $this->input->post('f_name'), "l_name" => $this->input->post('l_name'), "gender" => $this->input->post('gender'), "email" => $this->input->post('email'), "per_list" => $aclmaster, "per_name" => $per_name, "menu_option" => $acluser, "admin_status" => $this->input->post('admin_status'), "message_display" => "");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
            $this->load->view('admin_user/edit-admin-user', $data);
            $this->load->view('template/footer');
            ...
            $data1 = array("f_name" => $this->input->post('f_name'), "l_name" => $this->input->post('l_name'), "gender" => $this->input->post('gender'), "menu_option" => $menu_option, "euser" => $logged_user_id, "selcd_proj" => $selcd_pro, "edate" => date('Y-m-d'), "role_id" => $this->input->post('user_type'), "admin_status" => $this->input->post('admin_status'));
            $changepassword = '';
            $changepassword = $this->input->post('passwod');
            if (!empty($changepassword)) {
                $data2 = array("con_pass" => $changepassword, "edate" => date('Y-m-d'));
            ...
            if ($result == TRUE) {
                ...
                $data_web = $this->Admintype_model->fetch_webiste_under_user_active_proj_for_edit($role_id, $user_id, $edit_user_role_id);
                $data_web_sel = $this->Admintype_model->fetch_webiste_under_user_active_projselected($role_id, $user_id);
                $data2 = array("id" => $this->input->post('id'), "f_name" => $this->input->post('f_name'), "l_name" => $this->input->post('l_name'), "gender" => $this->input->post('gender'), "email" => $this->input->post('email'), "per_list" => $aclmaster, "per_name" => $per_name, "menu_option" => $acluser, "web_proj" => $data_web, "web_proj_sel" => $data_web_sel, "admin_status" => $this->input->post('admin_status'), "message_display" => "User information succuesfully updated");
                $this->load->view('template/header');
                $this->load->view('template/sidebar');
    ...
    public function update_user_profile() {
        ...
        $data_web_sel = $this->Admintype_model->usr_my_project();
        if ($this->form_validation->run() == FALSE) {
            $data = array("id" => $this->input->post('id'), "f_name" => $this->input->post('f_name'), "l_name" => $this->input->post('l_name'), "gender" => $this->input->post('gender'), "data_web_sel" => $data_web_sel, "email" => $this->input->post('email'), "message_display" => "");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
            $this->load->view('admin_user/user_profile', $data);
            $this->load->view('template/footer');
        } else {
            $data1 = array("f_name" => $this->input->post('f_name'), "l_name" => $this->input->post('l_name'), "gender" => $this->input->post('gender'), "euser" => $user_id, "edate" => date('Y-m-d'));
            $result = $this->Admintype_model->update_user_info($data1, $user_id);
            if ($result == TRUE) {
                $data2 = array("id" => $this->input->post('id'), "f_name" => $this->input->post('f_name'), "l_name" => $this->input->post('l_name'), "gender" => $this->input->post('gender'), "email" => $this->input->post('email'), "data_web_sel" => $data_web_sel, "message_display" => "information succuesfully updated");
                $this->load->view('template/header');
                $this->load->view('template/sidebar');
    ...
    public function web_registration_show() {
        ...
        if ($ch_permsn == '0') {
            ...
            $data = array("pname" => $this->input->post('pname'), "website" => $this->input->post('website'), "google_a_id" => $this->input->post('google_a_id'), "face_analytic_id" => $this->input->post('face_analytic_id'), "countery_info" => $countery_info, "currency_info" => $currency_info, "admin_status" => $this->input->post('admin_status'), "message_display" => "");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
    ...
    public function new_website_registration() {
        ...
        if ($ch_permsn == '0') {
            ...
            if ($this->form_validation->run() == FALSE) {
                //if validation fail from redirect to same page as selected data
                $data = array("pname" => $this->input->post('pname'), "website" => $this->input->post('website'), "google_a_id" => $this->input->post('google_a_id'), "face_analytic_id" => $this->input->post('face_analytic_id'), "addcountry" => $this->input->post('addcountry'), "time_zone" => $this->input->post('time_zone'), "cuurency_web" => $this->input->post('cuurency_web'), "web_status" => $this->input->post('web_status'), "message_display" => "");
                $this->load->view('template/header');
                $this->load->view('template/sidebar');
                $this->load->view('web_project/web-registration', $data);
                $this->load->view('template/footer');
                ...
                $data1 = array('pname' => $this->input->post('pname'), 'account_id' => $result1232, 'website' => $this->input->post('website'), 'google_a_id' => $this->input->post('google_a_id'), "face_analytic_id" => $this->input->post('face_analytic_id'), "country_code" => $this->input->post('addcountry'), "time_zone" => $this->input->post('time_zone'), "currency_id" => $this->input->post('cuurency_web'), "flag" => $this->input->post('web_status'), 'cdate' => date('Y-m-d'));
                // print_r($data1);
                // die;
                $result = $this->Admintype_model->web_registration_insert($data1);
                if ($result == TRUE) {
                    $cookie = array('name' => 'sucsess_cokiee', 'value' => 'Website added successfully', 'expire' => 7, 'prefix' => 'dash_',);
                    set_cookie($cookie);
    ...
    public function list_website() {
        ...
        if ($ch_permsn == '0') {
            ...
            $config = array('base_url' => base_url('admin/Dashboard/list_website'), 'per_page' => 10, 'total_rows' => $all_count, 'full_tag_open' => '<ul class="pagination">', 'full_tag_close' => '</ul>', 'first_link' => false, 'last_link' => false, 'first_tag_open' => '<li class="page-link">', 'first_tag_close' => '</li>', 'prev_link' => 'Previous ', 'prev_tag_open' => '<li class="page-link">', 'prev_tag_close' => '</li>', 'next_link' => ' Next', 'next_tag_open' => '<li>', 'next_tag_close' => '</li>', 'last_tag_open' => '<li>', 'last_tag_close' => '</li>', 'cur_tag_open' => '<li class="page-item active"><a class="page-link">', 'cur_tag_close' => '</a></li>', 'num_tag_open' => '<li class="page-link">', 'num_tag_close' => '</li>',);
            $this->pagination->initialize($config);
            $uri = $this->uri->segment(5);
    ...
    public function view_edit_proj_web() {
        ...
        if ($ch_permsn == '0' || $proj_id === NULL) {
            ...
            foreach ($result as $row) {
                $data = array("countery_info" => $countery_info, "currency_info" => $currency_info, "zone_info" => $zone_info, "id" => $row->id, "pname" => $row->pname, "accountnumber" => $row->accountnumber, "website" => $row->website, "google_a_id" => $row->google_a_id, "face_analytic_id" => $row->face_analytic_id, "google_av_id" => $row->analytics_view_id, "country_code" => $row->country_code, "time_zone" => $row->time_zone, "currency_id" => $row->currency_id, "flag" => $row->flag, "message_display" => "");
            }
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
    ...
    public function edit_admin_web_proj() {
        ...
        $this->form_validation->set_rules('time_zone', 'Time zone', 'required|is_natural');
        if ($this->form_validation->run() == FALSE) {
            $data = array("pname" => $this->input->post('pname'), "website" => $this->input->post('website'), "google_a_id" => $this->input->post('google_a_id'), "face_analytic_id" => $this->input->post('face_analytic_id'), "google_av_id" => $this->input->post('google_av_id'), "addcountry" => $this->input->post('addcountry'), "time_zone" => $this->input->post('time_zone'), "cuurency_web" => $this->input->post('cuurency_web'), "flag" => $this->input->post('web_status'), "message_display" => "");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
            $this->load->view('web_project/edit-website', $data);
            $this->load->view('template/footer');
        } else {
            $data1 = array('pname' => $this->input->post('pname'), 'google_a_id' => $this->input->post('google_a_id'), "face_analytic_id" => $this->input->post('face_analytic_id'), "analytics_view_id" => $this->input->post('google_av_id'), "country_code" => $this->input->post('addcountry'), "time_zone" => $this->input->post('time_zone'), "currency_id" => $this->input->post('cuurency_web'), "flag" => $this->input->post('web_status'), 'edate' => date('Y-m-d'), 'euser' => $logged_user_id);
            ...
            if ($result == TRUE) {
                ...
                $result = $this->Admintype_model->fetch_view_edit_web_project($proj_id);
                foreach ($result as $row) {
                    $data3 = array("countery_info" => $countery_info, "currency_info" => $currency_info, "zone_info" => $zone_info, "id" => $row->id, "pname" => $row->pname, "accountnumber" => $row->accountnumber, "website" => $row->website, "google_a_id" => $row->google_a_id, "face_analytic_id" => $row->face_analytic_id, "google_av_id" => $row->analytics_view_id, "country_code" => $row->country_code, "time_zone" => $row->time_zone, "currency_id" => $row->currency_id, "flag" => $row->flag, "message_display" => "Website Detail Successfully Updated");
                }
                $this->load->view('template/header');
                $this->load->view('template/sidebar');
    ...
    public function change_pass_user() {
        $logged_user_id = $this->session->userdata('user_id');
        $data3 = array("id" => $logged_user_id, "message_display" => "");
        $this->load->view('template/header');
        $this->load->view('template/sidebar');
    ...
    public function update_user_password() {
        ...
        $data1 = array("old_pass" => $this->input->post('old_pass'), "new_pass" => $this->input->post('new_pass'), "con_pass" => $this->input->post('con_pass'), "euser" => $user_id, "edate" => date('Y-m-d'));
        $result = $this->Admintype_model->update_user_info_pass($data1, $user_id);
        if ($result == TRUE) {
            $data2 = array("id" => $this->input->post('id'), "old_pass" => "", "new_pass" => "", "con_pass" => "", "message_display" => "Password Changed");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
            $this->load->view('admin_user/change-password', $data2);
            $this->load->view('template/footer');
        } else {
            $data2 = array("id" => $this->input->post('id'), "old_pass" => "", "new_pass" => "", "con_pass" => "", "message_display" => "Old Password Mismatch");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
            $this->load->view('admin_user/change-password', $data2);
            $this->load->view('template/footer');
