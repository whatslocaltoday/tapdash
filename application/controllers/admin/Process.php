...
class Process extends CI_Controller {
    ...
    public function list_plan_process() {
        ...
        if ($ch_permsn == '0') {
            ...
            if (isset($_POST['search']) || isset($_POST['from_date']) || isset($_POST['to_date']) || isset($_POST['proj_id'])) {
                ...
                $config = array('base_url' => base_url('admin/Process/list_plan_process'), 'per_page' => 30, 'total_rows' => $all_leads_count, 'full_tag_open' => '<ul class="pagination">', 'full_tag_close' => '</ul>', 'first_link' => false, 'last_link' => false, 'first_tag_open' => '<li class="page-link">', 'first_tag_close' => '</li>', 'prev_link' => 'Previous ', 'prev_tag_open' => '<li class="page-link">', 'prev_tag_close' => '</li>', 'next_link' => ' Next', 'next_tag_open' => '<li>', 'next_tag_close' => '</li>', 'last_tag_open' => '<li>', 'last_tag_close' => '</li>', 'cur_tag_open' => '<li class="page-item active"><a class="page-link">', 'cur_tag_close' => '</a></li>', 'num_tag_open' => '<li class="page-link">', 'num_tag_close' => '</li>',);
    ...
    public function fb_list_plan_process() {
        ...
        if ($ch_permsn == '0') {
            ...
            if (isset($_POST['search']) || isset($_POST['from_date']) || isset($_POST['to_date']) || isset($_POST['proj_id'])) {
                ...
                //$social_id='1';
                $all_leads_count = $this->Account_model->record_countview_project_plan_process($proj_id, $from_date, $to_date, $social_id);
                $config = array('base_url' => base_url('admin/Process/fb_list_plan_process'), 'per_page' => 30, 'total_rows' => $all_leads_count, 'full_tag_open' => '<ul class="pagination">', 'full_tag_close' => '</ul>', 'first_link' => false, 'last_link' => false, 'first_tag_open' => '<li class="page-link">', 'first_tag_close' => '</li>', 'prev_link' => 'Previous ', 'prev_tag_open' => '<li class="page-link">', 'prev_tag_close' => '</li>', 'next_link' => ' Next', 'next_tag_open' => '<li>', 'next_tag_close' => '</li>', 'last_tag_open' => '<li>', 'last_tag_close' => '</li>', 'cur_tag_open' => '<li class="page-item active"><a class="page-link">', 'cur_tag_close' => '</a></li>', 'num_tag_open' => '<li class="page-link">', 'num_tag_close' => '</li>',);
                $this->pagination->initialize($config);
                $uri = $this->uri->segment(5);
                $all_leads = $this->Account_model->view_project_plan_process($config["per_page"], $this->uri->segment(4), $proj_id, $from_date, $to_date, $social_id);
                $data['all_leads'] = $all_leads;
    ...
    public function add_plan_process() {
        ...
        if ($ch_permsn == '0') {
            ...
            $data = array("daily_budget" => $this->input->post('daily_budget'), "plan_cost" => $this->input->post('plan_cost'), "plan_view" => $this->input->post('plan_view'), "plan_trafic" => $this->input->post('plan_trafic'), "data_web_sel" => $data_web_sel, "plan_lead" => $this->input->post('plan_lead'), "message_display" => "");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
    ...
    public function new_plan_process_add() {
        ...
        if ($ch_permsn == '0') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data1 = array("buget" => $this->input->post('daily_budget'), "cost" => $this->input->post('plan_cost'), "social_id" => '1', "view" => $this->input->post('plan_view'), "traffic" => $this->input->post('plan_trafic'), "proj_id" => $this->input->post('web_id'), "lead" => $this->input->post('plan_lead'), "plan_date" => $this->input->post('date'), 'cdate' => date('Y-m-d'), 'cuser' => $logged_user_id);
    ...
    public function fb_new_plan_process_add() {
        ...
        if ($ch_permsn == '0') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data1 = array("buget" => $this->input->post('daily_budget'), "cost" => $this->input->post('plan_cost'), "social_id" => '2', "view" => $this->input->post('plan_view'), "traffic" => $this->input->post('plan_trafic'), "proj_id" => $this->input->post('web_id'), "lead" => $this->input->post('plan_lead'), "plan_date" => $this->input->post('date'), 'cdate' => date('Y-m-d'), 'cuser' => $logged_user_id);
                $result = $this->Account_model->insert_new_daily_plan_process($data1);
                if ($result == TRUE) {
                    $this->session->set_flashdata('success_message', 'Data Added');
    ...
    public function view_edit_plan_progress() {
        ...
        if ($ch_permsn == '0' || $plan_p_id === NULL) {
            ...
            foreach ($result as $row) {
                $data = array("id" => $row->id, "daily_budget" => $row->buget, "plan_cost" => $row->cost, "plan_view" => $row->view, "plan_trafic" => $row->traffic, "data_web_sel" => $row->proj_id, "website_url" => $row->website, "website_title" => $row->pname, "plan_lead" => $row->lead, "date" => $row->plan_date, "message_display" => "");
            }
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
    ...
    public function edit_plan_progress() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array("id" => $this->input->post('id'), "daily_budget" => $this->input->post('daily_budget'), "plan_cost" => $this->input->post('plan_cost'), "plan_view" => $this->input->post('plan_view'), "plan_trafic" => $this->input->post('plan_trafic'), "data_web_sel" => $data_web_sel, "website_url" => $website_url, "website_title" => $website_title, "plan_lead" => $this->input->post('plan_lead'), "date" => $this->input->post('date'), "message_display" => "");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
            $this->load->view('plan/edit_plan', $data);
            $this->load->view('template/footer');
        } else {
            $data1 = array("buget" => $this->input->post('daily_budget'), "cost" => $this->input->post('plan_cost'), "view" => $this->input->post('plan_view'), "traffic" => $this->input->post('plan_trafic'), "lead" => $this->input->post('plan_lead'), "plan_date" => $this->input->post('date'), 'edate' => date('Y-m-d'), 'euser' => $logged_user_id);
            ...
            if ($result == TRUE) {
                ...
                foreach ($result1 as $row) {
                    $data = array("id" => $row->id, "daily_budget" => $row->buget, "plan_cost" => $row->cost, "plan_view" => $row->view, "plan_trafic" => $row->traffic, "data_web_sel" => $row->proj_id, "website_url" => $row->website, "website_title" => $row->pname, "plan_lead" => $row->lead, "date" => $row->plan_date);
                }
                ...
                $data = array("id" => $this->input->post('id'), "daily_budget" => $this->input->post('daily_budget'), "plan_cost" => $this->input->post('plan_cost'), "plan_view" => $this->input->post('plan_view'), "plan_trafic" => $this->input->post('plan_trafic'), "data_web_sel" => $data_web_sel, "website_url" => $website_url, "website_title" => $website_title, "plan_lead" => $this->input->post('plan_lead'), "date" => $this->input->post('date'));
                            ...
                            if (count($data_csv3) == 7 && $count != 1) {
                                ...
                                if ($row1 > 2) {
                                    ...
                                    if ($result3) {
                                        ...
                                        $adword_camp_data = array('proj_id' => $proIDget, 'view' => $data_csv3[2], 'traffic' => $data_csv3[3], 'lead' => $data_csv3[4], 'cost' => $data_csv3[6], 'buget' => $data_csv3[5], 'plan_date' => $planDate, 'cuser' => $logged_user_id, 'social_id' => '1', 'cdate' => date('Y-m-d'));
                            ...
                            if (count($data_csv3) == 7 && $count != 1) {
                                ...
                                if ($row1 > 2) {
                                    ...
                                    if ($result3) {
                                        ...
                                        $planDate = date('Y-m-d', strtotime(trim($data_csv3[1])));
                                        $adword_camp_data = array('proj_id' => $proIDget, 'view' => $data_csv3[2], 'traffic' => $data_csv3[3], 'lead' => $data_csv3[4], 'cost' => $data_csv3[6], 'buget' => $data_csv3[5], 'plan_date' => $planDate, 'cuser' => $logged_user_id, 'social_id' => '2', 'cdate' => date('Y-m-d'));
                                        $result3 = $this->Account_model->insert_day_wise_adwordtata($adword_camp_data);
                                    }
                                }
                            }
    ...
    public function keyword_idea() {
        ...
        if ($ch_permsn == '0') {
            ...
            $role_id = $this->session->userdata('role_id');
            $data_web_sel = $this->Admintype_model->usr_my_project();
            $data = array("keyword_name" => $this->input->post('keyword_name'), "data_web_sel" => $data_web_sel, "message_display" => "");
            $this->load->view('template/header');
            $this->load->view('template/sidebar');
    ...
    public function get_keyword_idea() {
        ...
        if ($ch_permsn == '0' || $keyword_name == '') {
            ...
            if ($keywordresult == TRUE) {
                $data = array("keyword_name" => $this->input->post('keyword_name'), "keywordresult" => $keywordresult, "message_display" => "");
                $this->load->view('template/header');
                $this->load->view('template/sidebar');
                $this->load->view('plan/keyword_idea', $data);
                $this->load->view('template/footer');
