...
Class Login_model extends CI_Model {
    ...
    public function registration_insertclientadmin($data) {
        ...
        //for user table
        $insdatauser = array('f_name' => $data['f_name'], 'l_name' => $data['l_name'], 'u_email' => $data['u_email'], 'u_password' => md5($data['n_password']), 'date' => $data['cdate']);
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            // $insdataproject = array('pname' => $data['proj_name'], 'website' => $data['proj_website'], 'cdate' => $data['cdate'], 'cuser' => $user_id, 'accountnumber' => $rand_str);
            // $this->db->insert('project_details', $insdataproject);
            // $project_id = $this->db->insert_id();
            // for usergroup
            $insdatausergrp = array('user_id' => $user_id, 'role_id' => 5);
            $this->db->insert('users_groups', $insdatausergrp);
            //for project user
            // $insdataprojctusr = array('user_id' => $user_id, 'user_role_id' => 5, 'is_active' => 1, 'project_id' => $project_id);
            ...
            if ($query->num_rows() == 1) {
                ...
                $insdataacluser = array('user_id' => $user_id, 'user_role_id' => 5, 'acl' => $acl);
    ...
    public function registration_insertclientadmin_google($data) {
        // Query to check whether username already exist or not
        //for user table
        $insdatauser = array('f_name' => $data['f_name'], 'l_name' => $data['l_name'], 'u_email' => $data['u_email'], 'u_photo	' => $data['u_photo'], 'date' => $data['cdate']);
        // Query to insert data in database
        $this->db->insert('user', $insdatauser);
        if ($this->db->affected_rows() > 0) {
            $user_id = $this->db->insert_id();
    ...
    public function send_reset_password($data) {
        $rand_str = $this->randomPassword();
        $updt_pass = array('u_password' => md5($rand_str));
    ...
    public function login($data) {
        ...
        if ($query->num_rows() == 1) {
            ...
            if ($query1->num_rows() == 1) {
                ...
                $session_data = array('user_name' => $data[0]->f_name, 'uemail' => $data[0]->u_email,);
