...
Class Account_model extends CI_Model {
    ...
    //with date graph data

    public function fetch_graph_data_under_useraccount_with_seletdeddat($start_Date,$end_Date)
    {
        $user_id=$this->session->userdata('user_id');
        $role_id=$this->session->userdata('role_id');
        if($role_id !='1')
        {
            ...
            $where="project_users.is_active ='1' ".$project_query." and project_users.user_id =".$user_id." and plan_date between '".$start_Date."' and '".$end_Date."' and project_details.id !='' GROUP BY DATE(plan_date)";
        }
        if($role_id =='1')
        {
            if(!empty($_SESSION['projID']))
            ...
            $where="userplanprocess.id !='' ".$project_query." and plan_date between '".$start_Date."' and '".$end_Date."' GROUP BY DATE(plan_date)";
        }
        //$this->db->distinct();
        $this->db->select_sum('userplanprocess.view');
        $this->db->select_sum('userplanprocess.traffic');
